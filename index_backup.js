  ...
  */
  ...
  async function handleScheduleUpload(ctx, user, hasPhoto) {
      ...
      if (hasPhoto) {
          try {
              ...
              // Очищаем старое расписание перед сохранением нового
              await Homework.findOneAndUpdate(
                  { classKey },
                  { 
                      schedule_photo_id: photoId,
                      data: {}, // Очищаем старое ДЗ при обновлении расписания
                      updated_at: new Date()
                  },
              ...
              await ctx.reply("✅ *Расписание успешно обновлено!*\n\n" +
                  ...
                  `⚠️ Старое домашнее задание очищено.`, {
                  parse_mode: 'Markdown',
                  ...keyboard
              });
          } catch (error) {
  ...
  async function showTodayDZ(ctx) {
    ...
    // Определяем день недели и время для рекомендации
    const now = new Date();
    const dayOfWeek = now.getDay(); // 0 = Воскресенье, 1 = Понедельник, ...
    const hour = now.getHours();
    
    // Если воскресенье ИЛИ вечер (после 18:00), рекомендуем посмотреть ДЗ на завтра
    const recommendTomorrow = (dayOfWeek === 0) || (hour >= 18);
  ...
  // Очистка старого ДЗ (старше 14 дней) при запуске бота
  async function cleanupOldHomework() {
    try {
      ...
      for (const hw of homeworks) {
        const data = hw.data || {};
        const oldDates = Object.keys(data).filter(dateStr => {
          const date = new Date(dateStr);
        ...
        if (oldDates.length > 0) {
          ...
          hw.data = data;
          hw.updated_at = new Date();
          await hw.save();
          totalCleaned += oldDates.length;
        }
      }
      
      if (totalCleaned > 0) {
        console.log(`🧹 Очищено ${totalCleaned} старых записей ДЗ (старше 14 дней)`);
      }
    } catch (error) {
      console.error("⚠️ Ошибка очистки старого ДЗ:", error);
    }
  }
  ...
  async function startBot() {
    ...
    // Подключаемся к БД и очищаем старое ДЗ
    try {
      await connectDB();
      await cleanupOldHomework();
    } catch (e) {
